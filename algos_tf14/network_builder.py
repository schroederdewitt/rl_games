...
class DQNBuilder(NetworkBuilder):
    ...
    def load(self, params):
        ...
        self.normalization = params.get('normalization', None)
        self.mix_with_state = params['mix_with_state']
        if 'cnn' in params:
            self.has_cnn = True
            self.cnn = params['cnn']
        else:
    ...
    def build(self, name, **kwargs):
        ...
        with tf.compat.v1.variable_scope(name, reuse=reuse):
            ...
            if self.mix_with_state:
                state_out = kwargs.pop('state_inputs')
                if self.has_cnn:
                    cnn_args_state = {
                        'name': 'state_cnn',
                        'ctype': self.cnn['type'],
                    ...
                    state_out = self._build_conv(**cnn_args_state)
                    state_out = tf.contrib.layers.flatten(state_out)

                state_out = self._build_mlp('state_mlp', state_out, self.units, self.activation, self.initializer, self.regularizer)
                state_out = dense_layer(inputs=state_out, units=1,
                                          kernel_initializer=self.init_factory.create(**self.initializer),
                                          activation=tf.identity,
                                          kernel_regularizer=self.regularizer_factory.create(**self.regularizer),
                                          name='state_value')

            if not self.mix_with_state:
                if self.atoms == 1:
                    return tf.squeeze(q_values), None
                else:
                    return q_values, None
            else:
                if self.atoms == 1:
                    return tf.squeeze(q_values), state_out
                else:
                    return q_values, state_out