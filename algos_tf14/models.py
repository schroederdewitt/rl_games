...
class ModelA2C(BaseModel):
    ...
    def __call__(self, dict, reuse=False):
        ...
        if not is_train:
            neglogp = tf.nn.softmax_cross_entropy_with_logits_v2(logits=logits,
...
class ModelA2CContinuous(BaseModel):
    ...
    def __call__(self, dict, reuse=False):
        ...
        prev_actions_ph = dict['prev_actions_ph']
        is_train = prev_actions_ph is not None
        mu, sigma, value = self.network(name, inputs=inputs, actions_num=actions_num, continuous=True,
...
class ModelA2CContinuousLogStd(BaseModel):
    ...
    def __call__(self, dict, reuse=False):
        ...
        mean, logstd, value = self.network(name, inputs=inputs, actions_num=actions_num, continuous=True, is_train=True,
                                           reuse=reuse)
...
class LSTMModelA2CContinuousLogStd(BaseModel):
    ...
    def __call__(self, dict, reuse=False):
        ...
        mu, logstd, value, states_ph, masks_ph, lstm_state, initial_state = self.network(name=name, inputs=inputs,
...
class LSTMModelA2CContinuous(BaseModel):
    ...
    def __call__(self, dict, reuse=False):
        ...
        mu, var, value, states_ph, masks_ph, lstm_state, initial_state = self.network(name=name, inputs=inputs,
                                                                                      actions_num=actions_num,
                                                                                      games_num=games_num,
                                                                                      batch_num=batch_num,
                                                                                      continuous=True,
...
class LSTMModelA2C(BaseModel):
    ...
    def __call__(self, dict, reuse=False):
        ...
        logits, value, states_ph, masks_ph, lstm_state, initial_state = self.network(name=name, inputs=inputs,
                                                                                     actions_num=actions_num,
                                                                                     games_num=games_num,
                                                                                     batch_num=batch_num,
                                                                                     continuous=False,
...
class VDN_DQN(BaseModel):
    ...
    def __call__(self, dict):
        ...
        if is_double:
            ...
            # (bs, n_agents, actions_num)
            next_selected_actions_onehot = tf.reshape(tf.one_hot(next_selected_actions, actions_num),
                                                      [batch_size_ph, n_agents, actions_num])
            # (bs, n_agents, 1)
...
class IQL_DQN(BaseModel):
    ...
    def __call__(self, dict):
        ...
        return qvalues, tf.reshape(current_action_qvalues, [batch_size_ph, n_agents, 1]), \
               tf.reshape(next_obs_values_target, [batch_size_ph, n_agents, 1])
