...
from rl_games.algos_tf14.tf_moving_mean_std import MovingMeanStd as RunningMeanStd
...
class CentralValueTrain:
    def __init__(self, state_shape, num_agents, num_steps, num_actors, num_actions, model, config, writter):
        ...
        self.scope = 'central_v'
        self.state_shape = shape_whc_to_cwh(self.state_shape) 
        state_config = {
        ...
        if self.normalize_input:
            self.running_mean_std = RunningMeanStd(shape=self.state_shape, epsilon = 1e-5, decay = 0.99)
    ...
    def build_ph(self):
        self.obs_ph = tf.placeholder(tf.float32, (None, ) + self.state_shape, name = 'obs')
        self.target_obs_ph = tf.placeholder(tf.float32, (None, ) + self.state_shape, name = 'target_obs') 
        self.value_preds_batch = tf.placeholder(tf.float32, (None,), name='value_preds_batch')
        self.values = tf.placeholder(tf.float32, (None,), name='values')
        self.return_batch = tf.placeholder(tf.float32, (None,), name='return_batch')
        self.curr_e_clip = tf.placeholder(tf.float32, ())
        self.clip_value = tf.placeholder(tf.bool, shape=(), name="is_critic_clip")
    ...
    def build_graph(self, reuse):
        with tf.variable_scope(self.scope):
            ...
            value_pred_clipped = self.value_preds_batch + \
                        tf.clip_by_value(self.values - self.value_preds_batch, 
                                        -self.curr_e_clip, 
                                        self.curr_e_clip)

            raw_critic_losses = tf.square(self.values - self.return_batch)
            value_losses_clipped = tf.square(self.value_pred_clipped - self.return_batch)
            clip_critic_loss = tf.max(raw_critic_loss, value_losses_clipped)

            self.critic_loss = U.switch(self.clip_value, clip_critic_loss, raw_critic_losses)
    ...
    def train_net(self, input_dict):
        ...
        if self.is_rnn:
            ...
            for i in range(num_minibatches):
                ...
                values, _ = self.model()
                loss = critic_loss(value_preds_batch, values, e_clip, returns_batch, self.clip_value)
                loss = loss.mean()
